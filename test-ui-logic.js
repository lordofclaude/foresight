...
has("only the localhost E2E seam can bypass auth", /AUTH_TEST_BYPASS = \/\^\(localhost\|127\\\.0\\\.0\\\.1\)\$\/\.test\(location\.hostname\) && qs\.get\("e2e_auth"\) === "1"[\s\S]*const skip = AUTH_TEST_BYPASS/);
lacks("production guest entry is removed", /id="gateEnter"|enter as guest/);
has("Clerk auth closes the gate", /__onClerkAuthenticated === "function"[\s\S]*window\.__onClerkAuthenticated\(user\)/);
