...
| **REAL / SHIPPED** | Polymarket comparison | The deployed UI compares TxLINE StablePrice with like-for-like public Polymarket moneylines at the selected replay timestamp. The live relay discovers the exact event through Gamma and reads CLOB history without credentials. Missing or post-target history remains unavailable and is labeled as partial coverage; current prices are never substituted into an as-of comparison. | Foresight does not query positions, place orders, verify execution, or touch funds. |
...
The Worker is read-only with respect to Foresight users. Rate/concurrency accounting and upstream freshness telemetry are coordinated through a Durable Object; a compatibility fallback remains for environments without that binding. Production browser access is restricted to `https://foresight-txline.vercel.app` through the `ALLOWED_ORIGINS` allowlist.
